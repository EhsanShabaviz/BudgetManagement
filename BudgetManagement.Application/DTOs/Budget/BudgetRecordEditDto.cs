...
namespace BudgetManagement.Application.DTOs.Budget
{
    ...
    /// اعتبارسنجی‌ها با توجه به تنظیمات Fluent API در BudgetManagementDbContext
    /// اعمال شده‌اند (StringLength/Required)؛
    /// فیلدهای عددی اختیاری هستند.
    /// </summary>
    public class BudgetRecordEditDto
    {
        [Required, DisplayName("کد زیرپروژه")]
        public string SubProjectCode { get; set; } = string.Empty; // در UI فقط Readonly

        [Required(ErrorMessage = "عنوان قرارداد را وارد کنید"), StringLength(300), DisplayName("عنوان قرارداد")]
